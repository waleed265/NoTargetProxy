...
public class RestUtil {
	...
	///////////////////////
	public HttpResponse FixedUpdateOrgConfig(ServerProfile profile, String resource, String resourceId)
			throws IOException {

		//Get Product Management API call

		String requestUrl = profile.getHostUrl() + "/" + profile.getApi_version() + "/organizations/" + profile.getOrg()
				+ "/" + resource + "/" + URLEncoder.encode(resourceId, "UTF-8");


		HttpRequest restRequest = REQUEST_FACTORY.buildGetRequest(new GenericUrl(requestUrl));
		restRequest.setReadTimeout(0);
